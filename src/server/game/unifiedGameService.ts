...
export class UnifiedGameService {
   ...
   */
  private async getProviderCredentials(providerName: 'RAWG' | 'THEGAMESDB' | 'GMDB'): Promise<Record<string, any> | undefined> {
    try {
      const targetNames = providerName === 'GMDB' || providerName === 'THEGAMESDB'
        ? ['THEGAMESDB', 'GMDB']
        : ['RAWG'];
      ...
      const records = await db
        ...
        .where(sql`${systemIntegrations.provider} IN (${sql.join(targetNames.map(n => sql`${n}`), sql`, `)})`);
  ...
  /**
   * Retrieve Unified Game Details by internal ID, media DB ID, RAWG id/slug, or GMDB id
   */
  public async getGameDetails(idOrSlug: string): Promise<UnifiedGame | null> {
    ...
    // Check if idOrSlug is numeric Media ID from DB
    if (/^\d+$/.test(idOrSlug)) {
      ...
      if (extIds.length > 0) {
        ...
        for (const ext of extIds) {
          if (ext.provider === 'RAWG') rawgId = ext.externalId;
          if (ext.provider === 'THEGAMESDB' || ext.provider === 'GMDB') gmdbId = ext.externalId;
        }
      } else {
        rawgId = idOrSlug;
      }
    ...
    // 1. Fetch primary data concurrently from RAWG and GMDB
    const [rawgGame, gmdbGame] = await Promise.all([
      rawgId ? this.rawgProvider.getGame(rawgId, rawgCreds) : Promise.resolve(null),
      gmdbId ? this.gmdbProvider.getGame(gmdbId, gmdbCreds) : Promise.resolve(null),
    ]);

    // Fallback: If we only have RAWG data and no GMDB ID yet, attempt GMDB search to enrich
    let enrichedGmdbGame = gmdbGame;
    if (rawgGame && !gmdbGame && gmdbCreds) {
      try {
        ...
        for (const candidate of gmdbSearch.results) {
          ...
          if (match.confidence === 'HIGH') {
            enrichedGmdbGame = await this.gmdbProvider.getGame(candidate.externalId, gmdbCreds);
            if (enrichedGmdbGame) {
              // Persist mapping
              await db
    ...
    if (!rawgGame && !enrichedGmdbGame) {
      return null;
    }

    const primarySlug = rawgGame?.slug || enrichedGmdbGame?.slug || idOrSlug;

    // 2. Fetch supplemental data concurrently
    const [
      series,
      dlcs,
      screenshotsRawg,
      ...
      rawgGame ? this.rawgProvider.getSeries(primarySlug, rawgCreds) : Promise.resolve(null),
      rawgGame ? this.rawgProvider.getDLCs(primarySlug, rawgCreds) : Promise.resolve([]),
      rawgGame ? this.rawgProvider.getScreenshots(primarySlug, rawgCreds) : Promise.resolve([]),
      enrichedGmdbGame ? this.gmdbProvider.getScreenshots(enrichedGmdbGame.externalId, gmdbCreds) : Promise.resolve([]),
      rawgGame ? this.rawgProvider.getVideos(primarySlug, rawgCreds) : Promise.resolve([]),
      rawgGame ? this.rawgProvider.getStores(primarySlug, rawgCreds) : Promise.resolve([]),
      rawgGame ? this.rawgProvider.getCreators(primarySlug, rawgCreds) : Promise.resolve([]),
      rawgGame ? this.rawgProvider.getSimilar(primarySlug, rawgCreds) : Promise.resolve([]),
      GameTranslator.translateGame({
        provider: rawgGame ? 'RAWG' : 'THEGAMESDB',
        externalId: rawgGame?.externalId || enrichedGmdbGame?.externalId || idOrSlug,
        title: rawgGame?.title || enrichedGmdbGame?.title || 'Без названия',
        description: rawgGame?.description || enrichedGmdbGame?.description,
      }),
    ]);

    // Merge screenshots from RAWG + GMDB
    ...
    const unified = GameMatcher.mergeGames(rawgGame, enrichedGmdbGame, {
      series,
      dlcs,
      ...
      creators,
      similar,
      translatedTitle: translation.title !== (rawgGame?.title || enrichedGmdbGame?.title) ? translation.title : undefined,
  ...
  /**
   * Search games across RAWG and GMDB with deduplication and filters
   */
  public async searchGames(
    ...
    const cached = await this.getFromCache<{ results: UnifiedGameSummary[]; total: number; hasMore: boolean; page: number }>(cacheKey);
    if (cached) return cached.data;
    ...
    const [rawgRes, gmdbRes] = await Promise.all([
      this.rawgProvider.searchGames(query, filters, rawgCreds),
      gmdbCreds && query.trim() ? this.gmdbProvider.searchGames(query, filters, gmdbCreds) : Promise.resolve({ results: [], total: 0, hasMore: false }),
    ]);

    // Deduplicate and combine results
    const combinedMap = new Map<string, UnifiedGameSummary>();

    // 1. Process RAWG results
    for (const rg of rawgRes.results) {
      ...
      combinedMap.set(GameMatcher.normalizeTitle(rg.title), summary);
    }

    // 2. Process GMDB results (enrich or add if distinct)
    for (const gg of gmdbRes.results) {
      const norm = GameMatcher.normalizeTitle(gg.title);
      const existing = combinedMap.get(norm);

      if (existing) {
        // Enrich existing with GMDB poster/details if RAWG was missing
    ...
    const total = (rawgRes.total || 0) + (gmdbRes.total || 0);
    const hasMore = rawgRes.hasMore || gmdbRes.hasMore;
  ...
  /**
   * Catalog / browse games with deep filtering
   */
  ...
  public async getSeries(gameIdOrSlug: string): Promise<UnifiedSeries | null> {
    const rawgCreds = await this.getProviderCredentials('RAWG');
    return this.rawgProvider.getSeries(gameIdOrSlug, rawgCreds);
  }
  ...
  public async getDLCs(gameIdOrSlug: string): Promise<UnifiedDLC[]> {
    const rawgCreds = await this.getProviderCredentials('RAWG');
    return this.rawgProvider.getDLCs(gameIdOrSlug, rawgCreds);
  }
  ...
  public async getGameDiagnostic(idOrSlug: string): Promise<GameAdminDiagnostic | null> {
    ...
    return {
      ...
      providerHealth: {
        rawg: {
          ok: rawgHealth.ok,
          status: rawgHealth.ok ? 'Active' : 'Error',
          latencyMs: rawgHealth.latencyMs,
          lastError: rawgHealth.error,
        },
        gmdb: {
          ok: gmdbHealth.ok,
          status: gmdbHealth.ok ? 'Active' : 'Error',
      ...
      matchingDetails: 'Единая схема Dodik Tracker (RAWG + TheGamesDB / GMDB)',
